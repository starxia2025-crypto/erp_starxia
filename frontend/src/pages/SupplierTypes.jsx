...
const SupplierTypes = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API}/supplier-types`, formData, { withCredentials: true });
      toast.success("Tipo de proveedor creado");
      setDialogOpen(false);
      setFormData({ name: "", description: "" });
      fetchTypes();
    } catch (error) {
      toast.error("Error al crear tipo de proveedor");
    }
  };
  ...
  const handleDelete = async (typeId) => {
    if (window.confirm("¿Estás seguro de eliminar este tipo?")) {
      try {
        await axios.delete(`${API}/supplier-types/${typeId}`, { withCredentials: true });
  ...
  return (
    <Layout title="Tipos de Proveedor">
      <div className="space-y-6" data-testid="supplier-types-page">
        <div className="flex justify-end">
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button data-testid="add-supplier-type-btn">
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Nuevo Tipo de Proveedor</DialogTitle>
              </DialogHeader>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                <p>No hay tipos de proveedor registrados</p>
              </div>
            )}
          </CardContent>
        </Card>
