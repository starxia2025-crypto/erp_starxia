...
const Dashboard = () => {
  ...
  const statCards = [
    hasPermission(user, "clients.read") ? { title: "Clientes", value: stats?.clients_count || 0, icon: Users, color: "text-blue-500" } : null,
    hasPermission(user, "suppliers.read") ? { title: "Proveedores", value: stats?.suppliers_count || 0, icon: Truck, color: "text-purple-500" } : null,
    hasPermission(user, "products.read") ? { title: "Productos", value: stats?.products_count || 0, icon: Package, color: "text-green-500" } : null,
    hasPermission(user, "sales.read") ? { title: "Pedidos", value: stats?.orders_count || 0, icon: ShoppingCart, color: "text-orange-500" } : null,
  ].filter(Boolean);

  const financialCards = [
    hasPermission(user, "sales.read") ? { title: "Ventas Totales", value: formatCurrency(stats?.total_sales), icon: TrendingUp, color: "text-emerald-500" } : null,
    hasPermission(user, "purchases.read") ? { title: "Compras Totales", value: formatCurrency(stats?.total_purchases), icon: DollarSign, color: "text-red-500" } : null,
    hasPermission(user, "sales.read") ? { title: "Facturas Pendientes", value: stats?.pending_invoices || 0, icon: FileText, color: "text-yellow-500" } : null,
    hasPermission(user, "inventory.read") ? { title: "Stock Bajo", value: stats?.low_stock_count || 0, icon: AlertTriangle, color: "text-red-500" } : null,
  ].filter(Boolean);
